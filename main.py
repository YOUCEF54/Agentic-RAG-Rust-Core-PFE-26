...
def openrouter_chat(messages: List[dict], model_override: Optional[str] = None) -> tuple[str, Optional[str]]:
  payload = {
...
def backend_chat(messages: List[dict], model_override: Optional[str] = None) -> tuple[str, Optional[str]]:
  if API_TYPE == "open_router":
    return openrouter_chat(messages, model_override=model_override)
  return ollama_chat(model_override or OLLAMA_CHAT_MODEL, messages)


def build_agent_chat_fns(payload_chat_model: Optional[str]) -> tuple[
  ...
  if API_TYPE == "open_router":
    shared_model = payload_chat_model or OPENROUTER_CHAT_MODEL
    shared_chat_fn = lambda msgs: openrouter_chat(msgs, model_override=shared_model)
    return shared_chat_fn, shared_chat_fn, shared_chat_fn, shared_chat_fn

  return (
    lambda msgs: ollama_chat(REFINER_MODEL, msgs),
    lambda msgs: ollama_chat(GENERATOR_MODEL, msgs),
    lambda msgs: ollama_chat(EVALUATOR_MODEL, msgs),
    lambda msgs: ollama_chat(SELECTOR_MODEL, msgs),
  )
...
def health():
  ...
  return {
    ...
    "llm_default_model": OPENROUTER_CHAT_MODEL if API_TYPE == "open_router" else OLLAMA_CHAT_MODEL,
  }
...
def query(payload: QueryRequest):
  ...
  if payload.mode.lower() == "naive":
    ...
    naive_model = payload.chat_model or (OPENROUTER_CHAT_MODEL if API_TYPE == "open_router" else OLLAMA_CHAT_MODEL)
