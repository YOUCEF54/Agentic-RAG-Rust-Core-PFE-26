...
if __name__ == "__main__":
    ...
    if needs_rebuild:
        ...
        print(f"Loaded {len(pages)} pages in {(time.perf_counter()-t0)*1000:.2f}ms")

        t0 = time.perf_counter()
        dataset = chunk_texts(pages)
        print(f"Chunked into {len(dataset)} chunks in {(time.perf_counter()-t0)*1000:.2f}ms")
        if dataset:
